...
class InvocationHandler {
    ...
    construct(target, argArray, newTarget) {
        if (this.mockType === InternalMocker_1.MockType.Partial) {
            if (InternalMocker_1.GetInternalMockerSafe(this.constructorMock) !== null) {
    ...
    get(target, property, receiver) {
        if (property === InternalMocker_1.INTERNAL_MOCKER_NAME) {
            return this.internalMocker;
        }
        let realValue = Reflect.get(this.realObject, property, receiver);
        ...
        let newCachedField;
        const mockName = this.mockName !== null ? `${this.mockName}.${property.toString()}` : property.toString();
    ...
    ownKeys(target) {
        ...
        for (const key of normalTargetKeys) {
            if (key === InternalMocker_1.INTERNAL_MOCKER_NAME || key === "constructor") {
                continue;
            }
